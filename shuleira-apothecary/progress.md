...
3. 「なぜスマホの形か。あっていない」「素材箱にイラストが入ってる感じにしろ」
   「今風のゲームだ。機能を使っていないのに完成しました、じゃない」
   → スマホ縦画面（480px箱）を廃止し、横型フルスクリーンに変更。台を左カラム・
      素材棚を右カラムに再配置し、素材に分類別アイコンを追加、台の中身をアイコン
      チップの「器」表示にした（D16）。
   → 実装を見直し、250msごとに画面全体を作り直していたためCSSアニメーションが
      毎ティックリセットされ実質静止していた問題を発見・修正。装飾DOMは起動時に
      一度だけ作り、データ依存部分だけ差分更新する方式に変更（D17）。Playwrightで
      DOM要素が複数ティックにわたり同一のまま保持される（＝アニメーションが
      途切れない）ことを確認済み。
   → 画像アセット受け入れパイプライン（import.meta.glob、ファイル名規約）を用意。
      作者がキャラクター・背景画像をチャットに添付し次第、コード変更なしで反映される。
4. 「理想像はNTE（Neverness to Everness）の店長スペシャルだ。調べてくれ」
   → WebSearchで調査（Owner's Selection：客ごとの待ち時間ゲージ、固定レシピ、
      ノルマの常時表示、★評価画面。D18）。「客が1人ずつ」という誤読を作者に
      訂正してもらい、実際は同時多人数＋個別ゲージだったため現行の同時進行3人
      仕様はそのまま維持。
   → 客ごとの待ち時間ゲージ（上限90秒、切れたら素材を戻して帰る）、HUDへの
      本日の目標人数のライブ表示、日終了画面の★1〜3評価を追加。Playwrightで
      ゲージ表示・★3と★1の両パターンを確認済み。
5. 「アイテムも全て作る予定だよ？」
   → 素材22種の画像も、キャラクター・背景と同じ受け入れパイプラインで対応する
      ことに決定（D19）。`src/assets/materials/` を新設し、素材ID（M01〜M22）
      をファイル名とする方式に統一、対応表をREADMEに用意。棚のカード・台の
      器チップの両方に反映、画像が無い素材は既存の分類アイコンに自動
      フォールバックするため一部だけ揃っても崩れない。tsc/vite build/vitest
      いずれも通過確認済み（materialsフォルダが空の状態で検証）。
